...
class InvoiceItem extends React.Component {
  ...
  handleQuantityUpdate = (itemName, newItemLabel) => {
    ...
    const updatedItems = this.state.items.map((item) =>
      item.label === itemName ? { ...item, label: newItemLabel } : item
    );
...
class ItemRow extends React.Component {
  ...
  handleProductChange = (event) => {
    const newItemLabel = event.target.value;
    const itemName = this.props.item?.label || '';
  
    // Call the parent component's label update handler
    this.props.onQuantityUpdate(itemName, newItemLabel);
  };
